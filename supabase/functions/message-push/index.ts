...
const WEBHOOK_SECRET = Deno.env.get("MESSAGE_PUSH_WEBHOOK_SECRET") ?? "";
const APNS_API_URL =
...
async function fetchSenderLabel(
  ...
  return (
    data?.display_name || data?.username || data?.handle || "Someone"
  );
}
...
async function handleFriendRequestInsert(
  ...
  console.log(LOG, "friend request insert detected", {
    requestId: record.id,
    senderId: record.sender_id,
    receiverId: record.receiver_id,
  });
...
Deno.serve(async (req) => {
  if (req.method === "GET") {
    return new Response("message-push alive", { status: 200 });
  }
  ...
  if (WEBHOOK_SECRET) {
    const headerSecret = req.headers.get("x-message-push-secret") ?? "";
  ...
  try {
    ...
    console.log(LOG, "ignored event", { table, type: payload?.type });
